...
import { adminApi, memberApi, type Policy, type CreateLoanPayload, type MemberLoan, policiesApi } from "@/lib/api";
...
const RequestLoan = () => {
  ...
  useEffect(() => {
    ...
    policiesApi
      ...
      .catch(()=>{});
      memberApi.getContributions().then((cs)=>setTotalContributions(cs.reduce((sum, c) => sum + (c.contribution_amount || 0), 0))).catch(()=>{});
  }, []);

  const maxloan = policy ? totalContributions * policy.loan_multiplier : 0;
  const exceedingMax = policy != null && formData.loan_amount > maxloan;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const loan = await adminApi.requestLoan(formData);
      setSuccessLoan(loan);
      toast.success("Loan request submitted");
      setFormData({ loan_amount: 0, interest_rate: 0, repayment_period: 0 });
      loadLoans();
    } catch (err: unknown) {
      const msg = (err as { message: string }).message || "Failed to request loan";
      setErrorMsg(msg);
      toast.error(msg);
    } finally {
      setSubmitting(false);
